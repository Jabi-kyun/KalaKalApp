...
class _SignupPageState extends State<SignupPage> {
  ...
  Future<bool> _showTermsAgreementDialog() async {
    final result = await showDialog<bool>(
      context: context,
      barrierDismissible:
          false, // User must tap a button, can't tap outside to close
        ...
        content: Container(
          ...
          child: ListView(
            children: const [
              ...
              SizedBox(height: 16),
              Text(
                '1. Data Privacy: We collect your name, contact details, and location to facilitate scrap collection and bidding.',
                style: TextStyle(fontSize: 13),
              ),
              SizedBox(height: 8),
              Text(
                '2. Location Tracking: Collectors must enable GPS to receive nearby notifications.',
                style: TextStyle(fontSize: 13),
              ),
              SizedBox(height: 8),
              Text(
                '3. Transactions: Bids are binding once accepted. Both parties must confirm pickup.',
                style: TextStyle(fontSize: 13),
              ),
              SizedBox(height: 8),
              Text(
                '4. Community Standards: Fraudulent listings or harassment will result in account ban.',
                style: TextStyle(fontSize: 13),
              ),
            ],
          ),
        ...
        if (!serviceEnabled) {
          if (mounted) {
            ...
            TopSnackBar.show(
              context,
              message:
                  'Please enable Location Services to sign up as a Collector.',
              backgroundColor: Colors.red,
            );
          }
          return;
